...
func (_q *UserQuery) QueryScripts() *ScriptQuery {
...
func (_q *UserQuery) WithScripts(opts ...func(*ScriptQuery)) *UserQuery {
	query := (&ScriptClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withScripts; query != nil {
		if err := _q.loadScripts(ctx, query, nodes,
			func(n *User) { n.Edges.Scripts = []*Script{} },
			func(n *User, e *Script) { n.Edges.Scripts = append(n.Edges.Scripts, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadScripts(ctx context.Context, query *ScriptQuery, nodes []*User, init func(*User), assign func(*User, *Script)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(script.FieldUserID)
	}
	query.Where(predicate.Script(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ScriptsColumn), fks...))
	}))
...
func (_q *UserQuery) loadGames(ctx context.Context, query *GameQuery, nodes []*User, init func(*User), assign func(*User, *Game)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(game.FieldUserID)
	}
