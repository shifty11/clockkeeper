...
func (_q *GameQuery) QueryScript() *ScriptQuery {
...
func (_q *GameQuery) WithScript(opts ...func(*ScriptQuery)) *GameQuery {
	query := (&ScriptClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GameQuery) loadScript(ctx context.Context, query *ScriptQuery, nodes []*Game, init func(*Game), assign func(*Game, *Script)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "script_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
